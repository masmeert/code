...
function handle<S extends Schema.ConstraintDecoder<unknown>>(
  ...
  ipcMain.handle(channel, (event, arg: unknown) => {
    ...
    if (!window || !event.senderFrame?.url.startsWith(APP_URL)) throw new Error(`${channel} is only available to APCode windows`);
...
export function registerBridge(daemonToken: string | null) {
  ...
  handle(
    ...
    Schema.String,
    async (window, title) =>
      (await dialog.showOpenDialog(window, { title, message: title, properties: ["openDirectory"] })).filePaths[0] ?? null,
  );
  handle(
    "pick-files",
    Schema.String,
    async (window, title) =>
      (await dialog.showOpenDialog(window, { title, message: title, properties: ["openFile", "multiSelections"] })).filePaths,
  );
