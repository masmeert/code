...
export interface MorphPopoverMenuProps extends HTMLAttributes<HTMLDivElement> {
  children: ReactNode;
}

/** Stacks menu items inside a MorphPopoverContent with a consistent gap. */
export function MorphPopoverMenu({ className, children, ...props }: MorphPopoverMenuProps) {
  return (
    <div role="menu" className={cn("flex flex-col gap-0.5", className)} {...props}>
      {children}
    </div>
  );
}
