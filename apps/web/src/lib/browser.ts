...
function updateThread(threadId: string, update: (browser: ThreadBrowser) => ThreadBrowser) {
  setState({ ...state, threads: { ...state.threads, [threadId]: update(threadBrowser(threadId)) } });
}
...
export function normalizeUrl(input: string): string | null {
  ...
  const candidate = trimmed.includes("://") ? trimmed : `${isLocalUrl(`http://${trimmed}`) ? "http" : "https"}://${trimmed}`;
  try {
...
export function openTab(threadId: string, url = "") {
  ...
  updateThread(threadId, (current) => ({ open: true, tabs: [...current.tabs, tab], activeTabId: tab.id }));
}
  ...
  setState({
    ...
    threads: {
      ...
      [threadId]: {
        ...
        tabs,
        activeTabId: browser.activeTabId === tabId ? (tabs[Math.min(index, tabs.length - 1)]?.id ?? null) : browser.activeTabId,
      },
    },
  });
}
...
export function updateTab(threadId: string, tabId: string, patch: Partial<Pick<BrowserTab, "url" | "title">>) {
  updateThread(threadId, (current) => ({ ...current, tabs: current.tabs.map((tab) => (tab.id === tabId ? { ...tab, ...patch } : tab)) }));
}
...
export function showTab(threadId: string, tabId: string) {
  ...
  setState({ ...state, alive: [...state.alive.filter((entry) => entry.tabId !== tabId), { threadId, tabId }].slice(-8) });
}
...
export function tabForWebContents(webContentsId: number) {
  for (const [tabId, webview] of webviews) {
    ...
    const threadId = Object.keys(state.threads).find((id) => state.threads[id]!.tabs.some((tab) => tab.id === tabId));
