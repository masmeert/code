...
export function NotificationStack({
  ...
  if (!primaryItem) {
    return (
      <div
        className={cn(
          "flex w-full max-w-[22rem] items-center justify-center gap-2 rounded-2xl bg-muted/70 px-5 py-8 text-sm font-medium text-muted-foreground",
          className,
        )}
      >
  ...
  return (
    <motion.button
      ...
      onClick={handleClick}
      className={cn(
        "relative z-10 block w-full max-w-[22rem] cursor-pointer rounded-2xl text-left text-foreground outline-none",
    ...
    >
      ...
      <span className="absolute inset-x-0 bottom-0 block p-3">
        ...
        >
          <span
            className={cn(
              "grid size-7 shrink-0 place-items-center rounded-full bg-muted text-xs font-medium text-foreground",
              classNames?.count,
            )}
          >
            {items.length}
