...
export function MorphingSearch({
  ...
  const panelWidth = mounted
    ? Math.max(anchorRect.width, Math.min(448, window.innerWidth - anchorRect.left - 16))
    : anchorRect.width;
  ...
  const overlay = mounted
    ? createPortal(
        <div
          aria-hidden={!open}
          inert={!open}
          className="pointer-events-none fixed top-0 left-0 z-50 size-0"
        >
          <AnimatePresence
            initial={false}
            mode="popLayout"
          ...
          >
            {open ? (
              <motion.div key="morphing-search-overlay" className="fixed top-0 left-0 size-0">
                <button
                  type="button"
                    ...
                    <div className="flex h-10 min-w-0 flex-1 items-center">
                      <input
                        ...
                        aria-activedescendant={
                          filteredItems.length > 0 ? `${uid}-option-${activeIndex}` : undefined
                        }
                      ...
                      filteredItems.map((item, index) => {
                        ...
                        return (
                          <button
                            ...
                            className="relative flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left outline-none focus-visible:ring-4 focus-visible:ring-ring focus-visible:ring-inset"
                          >
                            {active ? (
                              <motion.span
  ...
  return (
    <LayoutGroup id={uid}>
      <div
        ref={anchorRef}
        className={cn("relative", iconOnly ? "size-12" : "h-12 w-72 max-w-full", className)}
      >
        {!open ? (
          <motion.button
            ref={triggerRef}
            ...
            className={cn(
              "flex size-full items-center rounded-xl bg-background/60 text-left backdrop-blur-md outline-none [--search-trigger-stroke:var(--color-border)] hover:[--search-trigger-stroke:var(--color-border-strong)] focus-visible:ring-4 focus-visible:ring-ring",
