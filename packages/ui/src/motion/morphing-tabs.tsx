      ...
      {active && surfaceHost && surfaceWidth > SURFACE_INSET * 2
        ? createPortal(
            <svg
              ...
              className={cn(
                "pointer-events-none absolute inset-x-0 top-0 h-[108px] w-full text-background",
                dragging ? "z-20" : "z-0",
                surfaceClassName,
              )}
            >
...
export function MorphingTabs({
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        "relative isolate min-w-0 overflow-hidden rounded-[2rem] bg-primary text-primary-foreground",
        classNames?.root,
        className,
      )}
    >
                  ...
                  >
                    <span
                      ...
                      className={cn(
                        ...
                        isActive
                          ? "bottom-0 rounded-t-[1.25rem] border-x-2 border-t-2 border-foreground/20"
                          : "bottom-2 rounded-[1rem] border-2 border-primary-foreground/60",
                      )}
                    />
                    {item.icon ? (
                      <span
